...
**Limite historique corrigée** ✅ FAIT (livré après ce lot) — le sélecteur de vague partagé (participant "Ma vague" + admin, `SkillUp.tsx`) réutilisait `/members/{discord_id}/vagues` (scope "vagues de l'appelant"), pas toutes les vagues du système. Migré vers `GET /vagues`, avec un correctif backend nécessaire : cet endpoint vivait dans `api/routers/admin.py` derrière `require_admin` (router-level), donc inutilisable tel quel pour un participant non-admin. Extrait dans un nouveau petit routeur `api/routers/vagues.py` (auth `require_api_key` + `X-Discord-Id` seulement, pas de check admin — lecture non sensible), les écritures (créer/activer/clôturer) restant admin-only dans `admin.py`. Frontend : `vagues` (état partagé) alimenté par `getSkillupVaguesAdmin()` au lieu de `getSkillupVagues()`, filtré sur `statut !== 'brouillon'`, `v.active` → `v.statut === 'active'` partout (`vagueParamFor`, les deux dropdowns).
